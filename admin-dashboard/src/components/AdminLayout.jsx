...
import { Menu, X, LayoutDashboard, Users, Calendar, User, LogOut, ChevronDown, Sun, Moon } from 'lucide-react'
...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <button onClick={() => setSidebarOpen(!sidebarOpen)} className="fixed top-2 left-2 z-50 p-2 bg-indigo-500 text-white rounded-lg lg:hidden hover:bg-indigo-600 transition-colors">
        {sidebarOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
      </button>

      <div className={`fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-slate-800 shadow-xl lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300`}>
        ...
        <nav className="p-4">
          {navItems.map(item => (
            <NavLink key={item.path} to={item.path} end={item.exact} onClick={() => setSidebarOpen(false)} className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg mb-2 transition-all duration-200 ${isActive ? 'nav-link-active' : 'nav-link'}`}>
            ...
            <div className="flex items-center gap-4 ml-auto">
              ...
              <div className="relative">
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="icon-btn flex items-center gap-2 text-slate-700 dark:text-slate-200">
                  <div className="w-8 h-8 bg-indigo-500 rounded-full flex items-center justify-center text-white font-medium">{user?.firstName?.charAt(0) || user?.name?.charAt(0)}</div>
                  <span className="hidden sm:inline text-sm">{getFullName()}</span>
