...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user.role === 'admin') { error('Use admin dashboard'); setLoading(false); return }
      ...
      <div className="relative w-full max-w-md">
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 p-6 sm:p-8 shadow-2xl">
          <div className="text-center mb-6 sm:mb-8">
            <div className="w-14 sm:w-16 h-14 sm:h-16 bg-gradient-to-br from-indigo-500 to-indigo-700 rounded-2xl flex items-center justify-center mx-auto mb-4">
              <GraduationCap className="w-7 sm:w-8 h-7 sm:h-8 text-white" />
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white">Student Login</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-10 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Your email" required />
              </div>
            </div>
            <div>
          ...
          <div className="mt-6 text-center">
            <p className="text-slate-400">Don't have an account? <Link to="/register" className="text-indigo-400 font-medium">Register</Link></p>
          </div>
        </div>
      </div>
    </div>
